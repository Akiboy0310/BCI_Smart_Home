...
def sameLen(df1,df2,df3,df4):
    # Find the length of the smallest data frame
    min_len = min(len(df) for df in [df1, df2, df3, df4])

    # Select the first n rows of each data frame
    con_df1 = df1.iloc[:min_len]
    con_df2 = df2.iloc[:min_len]
    con_df3 = df3.iloc[:min_len]
    con_df4 = df4.iloc[:min_len]
    return con_df1, con_df2, con_df3, con_df4
def concateDf(df1,df2,df3,df4):
    ...
    df1, df2, df3, df4 = sameLen(df1, df2, df3, df4)
